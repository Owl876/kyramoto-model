#все ориентируются на того кто спереди
...
def update():
    ...
    def dPhase(i, phase):
        ...
        for j, op in enumerate(allOperators):
            if j < i:
                d_phase += K*  math.exp((j-i)/5) * math.sin(op.phase - phase)
        return d_phase / (i+1)
...
k_slider = Slider(screen, left_x_border + 50, left_y_border + 50, 200, 20, min=0, max=20,
